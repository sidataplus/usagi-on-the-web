...
WORK_DIR="${ROOT_DIR}/temp/native-tachiom-dev-drugs-50-smoke"
PORT="${NATIVE_TACHIOM_DEV_DRUGS_50_SMOKE_PORT:-8795}"
BASE_URL="http://127.0.0.1:${PORT}"
FIXTURE_DIR="${ROOT_DIR}/fixtures/dev-drugs-50"
API_KEY="${NATIVE_TACHIOM_DEV_DRUGS_50_SMOKE_API_KEY:-local-native-tachiom-smoke}"
TACHIOM_TOTAL_CENTROIDS="${TACHIOM_TOTAL_CENTROIDS:-50}"

: "${TACHIOM_BUILD_BIN:?Set TACHIOM_BUILD_BIN to the upstream tachiom_build binary}"
: "${TACHIOM_SEARCH_BIN:?Set TACHIOM_SEARCH_BIN to the upstream tachiom_search binary}"
...
TACHIOM_ARTIFACT_ID="native-dev-drugs-50-tachiom-v1" \
TACHIOM_BACKEND=cli \
TACHIOM_BUILD_BIN="${TACHIOM_BUILD_BIN}" \
TACHIOM_TOTAL_CENTROIDS="${TACHIOM_TOTAL_CENTROIDS}" \
TACHIOM_NORMALIZE=true \
...
TACHIOM_SEARCH_BIN="${TACHIOM_SEARCH_BIN}" \
...
print(json.dumps({
    ...
    "status": "native_tachiom_expected_targets_in_candidates",
}))
PY
