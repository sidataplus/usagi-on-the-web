...
pub async fn run_worker() -> anyhow::Result<()> {
    ...
    let queue_names = config.queue_names();
        ...
        if let Some(job) = store.claim_next(&queue_names)? {
            ...
            if job.kind == JobKind::CatalogBuild {
                ...
                if let Ok(embeddings_path) = std::env::var("SAPBERT_PRECOMPUTED_EMBEDDINGS_PATH") {
                    ...
                    match build_sapbert_dense_index_from_precomputed(
                        SapbertPrecomputedBuildOptions {
                ...
                match validate_sapbert_model_artifact(SapbertModelArtifactPaths {
                    ...
                    Ok(artifact) => {
                        ...
                        match build_sapbert_dense_index(SapbertDenseBuildOptions {
                            ...
                            catalog_artifact_id: "local-catalog-standard-v1".to_string(),
                            model_artifact_id:
                                "cambridgeltl/SapBERT-UMLS-2020AB-all-lang-from-XLMR".to_string(),
                            documents,
                        }) {
                            Ok(summary) => {
                                set_stage(&store, &job.id, "validating_artifact", None)?;
                    ...
                    Ok(()) => {
                        ...
                        match write_thirawat_doc_embedding_artifact(
                            ThirawatDocEmbeddingBuildOptions {
                                ...
                                artifact_id: Some(
                                    "local-thirawat-drug-doc-embeddings-v1".to_string(),
                                ),
                                overwrite: true,
                            },
                            documents,
                ...
                match validate_thirawat_doc_embedding_artifact(ThirawatDocEmbeddingArtifactPaths {
                    ...
                    Ok(()) => {
                        let tachiom_index_dir =
                            PathBuf::from(std::env::var("TACHIOM_INDEX_DIR").unwrap_or_else(
                            ...
                            std::env::var("THIRAWAT_QUERY_EMBEDDINGS_PATH")
                        {
                            ...
                            MapperDrugBatchResponse {
                                ...
                                provenance: Provenance {
                                    ...
                                    model_artifact_id: Some(
                                        "sidataplus/THIRAWAT-SapBERT".to_string(),
                                    ),
            ...
            println!("no queued jobs for queues={}", config.queues);
            if config.run_once {
                break;
            }
...
impl WorkerConfig {
    fn from_env() -> anyhow::Result<Self> {
        let worker_queues_env = std::env::var("WORKER_QUEUES").ok();
        Self::from_args_and_env(std::env::args(), worker_queues_env.as_deref())
    }

    fn from_args_and_env<I, S>(args: I, worker_queues_env: Option<&str>) -> anyhow::Result<Self>
    where
        I: IntoIterator<Item = S>,
        S: Into<String>,
    {
        let mut queues = worker_queues_env
            .map(ToString::to_string)
            .unwrap_or_else(|| "catalog,index,embed,map".to_string());
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                "--queues" => {
                    queues = args
                        .next()
                        .ok_or_else(|| anyhow::anyhow!("--queues requires a value"))?;
                }
                value if value.starts_with("--queues=") => {
                    queues = value
                        .strip_prefix("--queues=")
                        .unwrap_or_default()
                        .to_string();
                ...
                "--help" | "-h" => {
                    return Err(anyhow::anyhow!(
                        "usage: usagi-worker --queues catalog,index,embed,map [--once]"
                    ));
                }
                other => return Err(anyhow::anyhow!("unrecognized argument {other}")),
            }
        }
        if queue_names_from(&queues).is_empty() {
            return Err(anyhow::anyhow!("at least one worker queue is required"));
        }
        Ok(Self { queues, run_once })
    }

    fn queue_names(&self) -> Vec<&str> {
        queue_names_from(&self.queues)
    }
}

fn queue_names_from(queues: &str) -> Vec<&str> {
    queues
        .split(',')
        .map(str::trim)
...
mod tests {
    ...
    #[test]
    fn worker_config_prefers_cli_queues_over_env() {
        let config = WorkerConfig::from_args_and_env(
            ["usagi-worker", "--queues", "map,index"],
            Some("catalog,embed"),
        )
        .unwrap();

        assert_eq!(config.queues, "map,index");
        assert_eq!(config.queue_names(), vec!["map", "index"]);
    }

    #[test]
    fn worker_config_supports_once_mode_for_smokes() {
        let config = WorkerConfig::from_args_and_env(
            ["usagi-worker", "--queues=map", "--once"],
            Some("catalog,embed"),
        )
        .unwrap();
